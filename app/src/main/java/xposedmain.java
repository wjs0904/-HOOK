...
{
    //按照实际使用情况修改下面几项的值
    private final String 当前模块包名 = "com.mycompany.application";
	private final String 实际hook逻辑处理类 = xposed.class.getName();
		...
		XposedHelpers.findAndHookMethod(Application.class, "attach", Context.class, new XC_MethodHook() {
					...
					protected void afterHookedMethod(MethodHookParam param) throws Throwable
					{
						...
						invokeHandleHookMethod(上下文, 当前模块包名, 实际hook逻辑处理类, 实际hook逻辑处理类的入口方法, loadPackageParam);			}else{
							new xposed().handleLoadPackage(loadPackageParam);
						}			}
					});}
